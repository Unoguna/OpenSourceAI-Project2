...
def _copy_matching_tensors(
    ...
    *,
    ...
    print(f"  {label}: copied {len(matched)}/{len(target_state)} tensors")
    return len(matched)


def _remap_discriminator_by_resolution(
    source_state: Mapping[str, torch.Tensor],
    source_cfg: DiscriminatorConfig,
    ...
    for key, value in source_state.items():
        if key.startswith("stages."):
            ...
            if len(parts) == 3 and parts[1].isdigit():
                source_stage = int(parts[1])
                if source_stage in stage_map:
                    key = f"stages.{stage_map[source_stage]}.{parts[2]}"
        remapped[key] = value
    return remapped
...
def init_from_baseline(
    ...
    """Warm-start from the distributed 256 ckpt.

    Exact 256 configs still load fully. Scaled 512/1024 configs copy the shared
    low-resolution generator trunk and remap matching discriminator stages.
    Newly-added high-resolution blocks stay randomly initialized.
    """
    print(f"Initializing from baseline: {init_path}")
    ckpt = torch.load(init_path, map_location=device, weights_only=False)

    g_copied = _copy_matching_tensors(G, ckpt["G_state"], label="G_state")
    ema_copied = _copy_matching_tensors(
        G_ema.shadow, ckpt["G_ema_state"], label="G_ema_state"
    )

    d_source_cfg = ckpt.get("meta", {}).get("discriminator_config")
    if d_source_cfg is None:
        from src.model import BASELINE_256_DISCRIMINATOR_CONFIG

        source_cfg = BASELINE_256_DISCRIMINATOR_CONFIG
    else:
        source_cfg = DiscriminatorConfig.from_dict(d_source_cfg)
    d_remapped = _remap_discriminator_by_resolution(ckpt["D_state"], source_cfg, D.cfg)
    d_copied = _copy_matching_tensors(D, d_remapped, label="D_state")

    if min(g_copied, ema_copied, d_copied) == 0:
        raise RuntimeError(
            "Baseline warm-start copied no tensors for at least one model. "
            "Check that the new config shares the 256 baseline trunk."
        )
